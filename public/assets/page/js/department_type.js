var baseURL = "http://dev.choray.tekmedi.com/";
...
var loadData = function () {
    pTable = $('#departmentTypeTable').DataTable({
        ...
        bAutoWidth: true,
        ajax: {
            "url": baseURL + "api/ListValue/get-all-list-values/" + listValueTypecode,
            "type": "GET",
...
$(document).on('click', '#btnEdit', function () {
    resetValidation();
    LoadAjaxAuth('GET', baseURL + "api/ListValue/get/" + $(this).attr('data-id'), {
...
$(document).on('click', '#btnActive', function () {
    LoadAjaxAuth('POST', baseURL + "api/ListValue/change-active", {
...
$('#btnSaveModal').click(function () {
    ...
    var validation = Array.prototype.filter.call(forms, function (form) {
        ...
        LoadAjaxAuth('POST', baseURL + "api/ListValue/check-code-unique", {
            ...
            if (status == 'success') {
                if (!data.result) {
                    ...
                    if (id === null || id === undefined || id.length === 0) {
                        LoadAjaxAuth('POST', baseURL + "api/ListValue/add", {
                            Code: $('#departmentTypeModal input[name="code"]').val(),
                        ...
                        LoadAjaxAuth('PUT', baseURL + "api/ListValue/update", {
                            Id: id,
                            Code: $('#departmentTypeModal input[name="code"]').val(),
                            Description: $('#departmentTypeModal input[name="description"]').val(),
                            DisplayOrder: 0,
...
$('#btnDeleteModal').click(function () {
    LoadAjaxAuth('DELETE', baseURL + "api/ListValue/delete/" + $('#confirm-delete input[name="id"]').val(), {
    }, getCookie('_tk'), function (data, status) {
        if (status == 'success') {
...
function getListValueByCode(code) {
    LoadAjaxAuth('GET', baseURL + "api/ListValueType/get-list-value-type/" + code, {
    }, getCookie('_tk'), function (data, status) {
        if (status == 'success') {
            var result = data.result;
