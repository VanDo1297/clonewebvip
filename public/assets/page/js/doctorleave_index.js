        ...
        columns:
            [
...

                {
                    ...
                    "sClass": "text-center",
                    "mRender": function (data, type, full, meta) {
                        return '<a class="btn btn-primary btn-sm btn--icon-text btnEdit" data-id="' + full.id + '" hidden><i class="zmdi zmdi-edit zmdi-hc-fw"></i> Chỉnh sửa</a>';
                    }
                }
...
//        $('#modal-default input[name="Notes"]').val(r.notes);
//        $('#modal-default input[name="StartTime"]').val(r.strStartTime).trigger('change');
//        $('#modal-default input[name="EndTime"]').val(r.strEndTime).trigger('change');
...
$(document).ready(function () {
    ...
    $('.btnSaveModal').click(function () {
        ...
        LoadAjaxAuth('POST', id === null || id === undefined || id.length === 0 ? APP_CONFIG.AddRow : APP_CONFIG.UpdateRow, {
            ...
            Notes: $('#modal-default input[name="Notes"]').val(),
            StartTime: $('#modal-default input[name="StartTime"]').val(),
            EndTime: $('#modal-default input[name="EndTime"]').val(),
        }, getCookie('_tk'), function (d, s) {
            if (d.hasError === true)
