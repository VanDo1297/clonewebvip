var listValueTypecode = "CV";
var listValueTypeId = "";
var loadData = function () {
    pTable = $('#positionTable').DataTable({
...
$(document).on('click', '#btnEdit', function () {
    ...
    LoadAjaxAuth('GET', APP_CONFIG.GetbyId + $(this).attr('data-id'), {
        ...
        if (status == 'success') {
            ...
            $('#positionModal input[name="id"]').val(result.id);
            $('#positionModal input[name="code"]').val(result.code);
            $('#positionModal input[name="description"]').val(result.description);
            $('#positionModal input[name="is_Active"]').prop('checked', result.is_active);
            $('#positionModal input[name="is_System"]').prop('checked', result.is_system);
...
$('#btnSaveModal').click(function () {
    ...
    var validation = Array.prototype.filter.call(forms, function (form) {
        ...
        LoadAjaxAuth('POST', APP_CONFIG.CheckCodeUnique, {
            Id: $('#positionModal input[name="id"]').val() == "" ? "1b287f11-f09d-4b55-9614-6ba2d0fd184f" : $('#positionModal input[name="id"]').val(),
            Code: $('#positionModal input[name="code"]').val()
        }, getCookie('_tk'), function (data, status) {
            if (status == 'success') {
                if (!data.result) {
                    var id = $('#positionModal input[name="id"]').val();
                    if (id === null || id === undefined || id.length === 0) {
                        LoadAjaxAuth('POST', APP_CONFIG.Add, {
                            Code: $('#positionModal input[name="code"]').val(),
                            Description: $('#positionModal input[name="description"]').val(),
                            DisplayOrder: 0,
                            IsActive: $('#positionModal input[name="is_Active"]').prop('checked'),
                            IsSystem: $('#positionModal input[name="is_System"]').prop('checked'),
                    ...
                    $("#validationCode").html("Mã chức vụ đã tồn tại trong hệ thống");
...
function clearData(modalName) {
    $(modalName)
        ...
        .val('')
        .end();
    $('#positionModal input[name="is_Active"]').prop('checked', true);
    $('#positionModal input[name="is_System"]').prop('checked', true);
...
function resetValidation() {
    ...
    $("#validationCode").html("Vui lòng nhập mã chức vụ");
}
