var listValueTypecode = "DTUT";
var listValueTypeId = "";
var loadData = function () {
    pTable = $('#priorityObjectTable').DataTable({
...
$(document).on('click', '#btnEdit', function () {
    ...
    LoadAjaxAuth('GET', APP_CONFIG.GetbyId + $(this).attr('data-id'), {
        ...
        if (status == 'success') {
            ...
            $('#priorityObjectModal input[name="id"]').val(result.id);
            $('#priorityObjectModal input[name="code"]').val(result.code);
            $('#priorityObjectModal input[name="description"]').val(result.description);
            $('#priorityObjectModal input[name="is_Active"]').prop('checked', result.is_active);
            $('#priorityObjectModal input[name="is_System"]').prop('checked', result.is_system);
            $('#priorityObjectModal').modal({ backdrop: 'static', keyboard: false });
        }
    });
});
...
$('#btnSaveModal').click(function () {
    ...
    var validation = Array.prototype.filter.call(forms, function (form) {
        ...
        LoadAjaxAuth('POST', APP_CONFIG.CheckCodeUnique, {
            Id: $('#priorityObjectModal input[name="id"]').val() == "" ? "1b287f11-f09d-4b55-9614-6ba2d0fd184f" : $('#priorityObjectModal input[name="id"]').val(),
            Code: $('#priorityObjectModal input[name="code"]').val()
        }, getCookie('_tk'), function (data, status) {
            if (status == 'success') {
                if (!data.result) {
                    var id = $('#priorityObjectModal input[name="id"]').val();
                    if (id === null || id === undefined || id.length === 0) {
                        LoadAjaxAuth('POST', APP_CONFIG.Add, {
                            Code: $('#priorityObjectModal input[name="code"]').val(),
                            Description: $('#priorityObjectModal input[name="description"]').val(),
                            DisplayOrder: 0,
                            IsActive: $('#priorityObjectModal input[name="is_Active"]').prop('checked'),
                            IsSystem: $('#priorityObjectModal input[name="is_System"]').prop('checked'),
                    ...
                    $("#validationCode").html("Mã đối tượng ưu tiên đã tồn tại trong hệ thống");
...
function clearData(modalName) {
    $(modalName)
        ...
        .val('')
        .end();
    $('#priorityObjectModal input[name="is_Active"]').prop('checked', true);
    $('#priorityObjectModal input[name="is_System"]').prop('checked', true);
...
function resetValidation() {
    ...
    $('#priorityObjectModal').modal('hide');
    $("#validationCode").html("Vui lòng nhập mã đối tượng ưu tiên");
}
