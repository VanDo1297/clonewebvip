var listValueTypecode = "CD";
var listValueTypeId = "";
var loadData = function () {
    pTable = $('#titleTable').DataTable({
...
$(document).on('click', '#btnEdit', function () {
    ...
    LoadAjaxAuth('GET', APP_CONFIG.GetbyId + $(this).attr('data-id'), {
            ...
            if (status == 'success') {
                ...
                $('#titleModal input[name="id"]').val(result.id);
                $('#titleModal input[name="code"]').val(result.code);
                $('#titleModal input[name="description"]').val(result.description);
                $('#titleModal input[name="is_Active"]').prop('checked', result.is_active);
                $('#titleModal input[name="is_System"]').prop('checked', result.is_system);
...
$('#btnSaveModal').click(function () {
    ...
    var validation = Array.prototype.filter.call(forms, function (form) {
        ...
        LoadAjaxAuth('POST', APP_CONFIG.CheckCodeUnique, {
            Id: $('#titleModal input[name="id"]').val() == "" ? "1b287f11-f09d-4b55-9614-6ba2d0fd184f" : $('#titleModal input[name="id"]').val(),
                Code: $('#titleModal input[name="code"]').val()
            }, getCookie('_tk'), function (data, status) {
                if (status == 'success') {
                    if (!data.result) {
                        var id = $('#titleModal input[name="id"]').val();
                        if (id === null || id === undefined || id.length === 0) {
                            LoadAjaxAuth('POST', APP_CONFIG.Add, {
                                Code: $('#titleModal input[name="code"]').val(),
                                Description: $('#titleModal input[name="description"]').val(),
                                DisplayOrder: 0,
                                IsActive: $('#titleModal input[name="is_Active"]').prop('checked'),
                                IsSystem: $('#titleModal input[name="is_System"]').prop('checked'),
                        ...
                        $("#validationCode").html("Mã chức danh đã tồn tại trong hệ thống");
...
function clearData(modalName) {
    $(modalName)
        ...
        .val('')
        .end();
    $('#titleModal input[name="is_Active"]').prop('checked', true);
    $('#titleModal input[name="is_System"]').prop('checked', true);
...
function resetValidation() {
    ...
    $("#validationCode").html("Vui lòng nhập mã chức danh");
}
