    ...
    pTable = $('#MainTable').DataTable({
        ...
        columns:
            [
                ...
                { "title": "Loại", "data": "type", "bSearchable": true, "bSortable": true },
                { "title": "Quầy thuốc", "data": "stores", "bSearchable": true, "bSortable": true },
                {
                    "title": "Chức năng",
                    "data": "id",
                    "bSearchable": false,
...
$(document).on('click', '#btnEdit', function () {
    ...
    LoadAjaxAuth('GET', url, {}, getCookie('_tk'), function (d, s) {
        var result = d.result;
        $('#pharmacy-modal input[name="id"]').val(result.id);
        $('#pharmacy-modal input[name="code"]').val(result.code);
        $('#pharmacy-modal input[name="name"]').val(result.name);
        $('#pharmacy-modal input[name="type"]').val(result.type);
        $('#pharmacy-modal input[name="stores"]').val(result.stores);
        $('#pharmacy-modal input[name="isActive"]').prop('checked', result.isActive);
        $('#pharmacy-modal').modal({ backdrop: 'static', keyboard: false });
    });
});
...
$('#btnSaveModal').click(function () {
    ...
    var validation = Array.prototype.filter.call(forms, function (form) {
        ...
        var url = APP_CONFIG.CheckCodeUnique + "?id=" + $('#pharmacy-modal input[name="id"]').val() + "&code=" + $('#pharmacy-modal input[name="code"]').val();
        LoadAjaxAuth('GET', url, {}, getCookie('_tk'), function (d, s) {
            if (d.success === false)
                setAlert(d.result.errors, 'Lỗi: ', '');
            else {
                if (!d.result) {
                    var id = $('#pharmacy-modal input[name="id"]').val();
                    LoadAjaxAuth(id === null || id === undefined || id.length === 0 ? 'POST' : 'PUT',
                        ...
                        Code: $('#pharmacy-modal input[name="code"]').val(),
                        Name: $('#pharmacy-modal input[name="name"]').val(),
                        Type: $('#pharmacy-modal input[name="type"]').val(),
                        Stores : $('#pharmacy-modal input[name="stores"]').val(),
                        isActive: $('#pharmacy-modal input[name="isActive"]').prop('checked')
                    ...
                    $("#validationCode").html("Mã nhà thuốc đã tồn tại trong hệ thống");
...
function clearData(modalName) {
    ...
    $('#pharmacy-modal input[name="isActive"]').prop('checked', true);
}
...
function resetValidation() {
    ...
    $("#validationCode").html("Vui lòng nhập mã nhà thuốc");
}