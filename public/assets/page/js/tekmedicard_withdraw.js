...
$(document).on('click', '#SaveBtn', function () {
    ...
    LoadAjaxAuth('POST', APP_CONFIG.GetFinally, {
        Patient_Code: $('#PaymentForm input[name="code"]').val(),
        Requested_Date: new Date()
    }, getCookie('_tk'), function (data, status) {
        if (status != 'success' && data.status == 400) {
            if (price > currentPrice) {
                toastr.error('Không thể rút số tiền lớn hơn số tiền trong thẻ');
                $('#PaymentForm input[name="currency"]').focus();
                return;
            }

            if (currentPrice - price < 10000) {
                toastr.error('Số tiền còn lại trong thẻ tối thiểu là 10.000');
                $('#PaymentForm input[name="currency"]').focus();
                return;
            }
            ...
            $('#confirm-withdraw').modal({ backdrop: 'static', keyboard: false });
        }
        else {
            toastr.error('Không thể rút tiền khi chưa thực hiện tất toán');
        }
    });
});
...
$('#btnSaveModal').click(function () {
    ...
    LoadAjaxAuth('POST', APP_CONFIG.Withdraw, {
...
$("#confirm-withdraw").on('hide.bs.modal', function(){
    reset();
});
...
function createTemplate(data) {
    ...
    return createWithdrawTemplate(templateData);
}
